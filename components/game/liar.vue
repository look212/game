  ...
  <div class="game-in" v-if="liarMode">
        ...
        <swiper ref="swiper" class="custom" :options="swiperOptions">
          <swiper-slide v-for="index in game.liar.participants" :key="`game_${index}`">
            ...
            <p v-else-if="liarMode === 'spy' && index === game.liar.spy">
              ...
              제시어는 <span class="quiz">{{ gameList[game.liar.quiz[0]].value }}</span
              >입니다.
            </p>
            <p v-else-if="liarMode === 'fool' && index === game.liar.liar">
              당신은 <span class="liar">일반 시민</span>입니다.<br />
              제시어는 <span class="quiz">{{ gameList[game.liar.quiz[1]].value }}</span
              >입니다.
            </p>
            <p v-else>
              당신은 <span class="liar">일반 시민</span>입니다.<br />
    ...
    <div class="result-wrap" v-else>
      <template v-if="!isLiarTrue">
        ...
        <div class="liar-list">
          <g-button
            v-for="index in game.liar.participants"
            :key="`liar_${index}`"
            :is-gray="true"
            @click="checkLiar(index)"
            >{{ index + 1 }}
          </g-button>
        </div>
      </template>
      <template v-if="isLiarTrue">
        ...
        <ul class="result-ul">
          <li
            v-for="(option, index) in gameList"
            :key="`result_${index}`"
