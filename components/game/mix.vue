<template>
  <div class="game-in" v-if="gameList.length > 1">
    ...
    <div class="controls">
      <g-button :is-gray="true" @click="handlePlay" :class="{ 'is-active': type === 'play' }"
        ><img src="../../static/images/icon/play.png" alt="play" height="14px;" />️</g-button
      >
      <g-button :is-gray="true" @click="handleStop" :class="{ 'is-active': type === 'stop' }"
        ><img src="../../static/images/icon/stop.png" alt="stop" height="14px"
      /></g-button>
    </div>
      ...
      <swiper ref="swiper" class="custom" :options="swiperOptions">
        <swiper-slide v-for="(game, index) in gameList" :key="`game_${index}`">
          <div v-if="index === activeIndex">
            <div v-for="(music, musicIndex) in game" :key="`music_${musicIndex}`">
              <p style="font-size: 1.2em">{{ music.singer }} - {{ music.value }}</p>
              <client-only>
                <youtube
                  :video-id="music.youtube_id"
                  :player-vars="{ start: music.start }"
                  player-width="100%"
                  player-height="56.25%"
                  @ready="ready"
                  @playing="playing($event, type)"
                ></youtube>
              </client-only>
            </div>
          </div>
