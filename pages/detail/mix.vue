<template>
  <div class="detail-wrap">
    ...
    <template v-if="!isGameStart">
      <div class="contents">
        <h2>
          노래 믹스 갯수 <span v-if="mixedNum">({{ mixedNum }}개)</span>
        </h2>
        <div class="btn-wrap">
          <g-button
            v-for="(option, index) in mixedList"
            :is-gray="true"
            :key="`mixed_number_${index}`"
            :class="{ 'is-active': mixedNum === option.value }"
            @click="setMixedNumber(option.value)"
      ...
      const methods = {
        setGameStart() {
          ...
          root.$swal('Game Start 😆').then(() => {
            game.setGameStart({
              subject: 'mix',
              questionCount: questionCount.value * mixedNum.value,
            })
          })
        },
      }
